...
//	Query := fmt.Sprintf(`SELECT * FROM public."%v"`, "REGEXTKUS")
	Query2 := `SELECT * FROM public."REGEXTKUS" WHERE "Categoria" = 'Respuesta';`
	stmt, err := BasePosGres.Prepare(Query2)
	if err != nil {
		fmt.Println(err)
		return expresion, err
	}
...
func CountAll() (int, error) {
	...
	Query := fmt.Sprintf(`SELECT count(*) FROM public."%v" WHERE "Categoria" = 'Respuesta';`, "REGEXTKUS")
	stmt, err := BasePosGres.Prepare(Query)
	if err != nil {
...
func GeneraTemplatesBusqueda(Expresions []ExpresionMgo) (string, string) {
	...
	for k, v := range Expresions {
		cuerpo += `<tr id = "` + v.ID.Hex() + `" onclick="window.location.href = '/Respuestas/detalle/` + v.ID.Hex() + `';">`
